...
if(isset($_POST['Save']))
{
  $folder_path = 'BrandIcon/';
  $Brand_Name = $_POST['Brand_Name'];

    $filename_Brand_Logo = basename($_FILES['Brand_Logo']['name']);
		$imageFileBrand_Logo = strtolower(pathinfo($filename_Brand_Logo,PATHINFO_EXTENSION));
    $newname_Brand_Logo =$folder_path.$Brand_Name.'.'.$imageFileBrand_Logo;


  $Check_Brand=mysqli_query($conn, "SELECT * FROM brand_tb WHERE Brand_Name='$Brand_Name'")or die('Error In Session');
  $Brand_Check=mysqli_fetch_array($Check_Brand);
  $Brand_CheckID = $Brand_Check['Brand_ID'];



  if($Brand_CheckID == "")
  {
      $Check_ID = "Select * From brand_tb ORDER BY Brand_ID DESC LIMIT 1";
      $result_Check_ID =mysqli_query($conn,$Check_ID);
      $row_Check_ID =mysqli_fetch_array($result_Check_ID);
      $Last_id = $row_Check_ID['Brand_ID'];
      ...
      $Brand_ID = $ref.''.$ars;
        ...
        $SQL_AddAcc ="INSERT INTO `brand_tb` (`Brand_ID`,`Brand_Name`,`Brand_Logo`) VALUES ('$Brand_ID', '$Brand_Name','$newname_Brand_Logo')";
        if ($conn->query($SQL_AddAcc)==TRUE)
          {
                if (move_uploaded_file($_FILES['Brand_Logo']['tmp_name'], $newname_Brand_Logo))
										{
                        echo '<script type="text/javascript">';
                        echo 'alert("บันทึกข้อมูล Brand เรียบร้อย");';
...
if(isset($_POST['BrandDelete']))
{
  ...
  $SQL_Brand = "DELETE FROM Brand_tb WHERE Brand_ID ='$Brand_ID';";
          if ($conn->query($SQL_Brand)==TRUE) {
            echo '<script type="text/javascript">';
            echo 'window.location.href = "Management-Create-Brand.php";';
            echo '</script>';
          }
}
  ...
  <div class="w3-row">
    ...
    <div class="w3-col m7">
      ...
      <div class="w3-container w3-card w3-white w3-round w3-margin"><br>
        <h2 class="w3-opacity" align="center">สร้างรายการ ยี่ห้อ อุปกรณ์</h2>
        <hr class="w3-clear">

              <form action="Management-Create-Brand.php" method="POST" enctype="multipart/form-data">
                <div class="w3-container w3-padding">
                      <p>ชื่อยี่ห้อ
                      <input type="text" class="form-control" placeholder="ชื่อยี่ห้อ" id="Brand_Name" name="Brand_Name" value=""></p>
                      
                      <p>สัญลักษณ์ยี่ห้อ <br>
                          <input type="file"   name="Brand_Logo" class="input-large" accept=".jpg,.jpeg,.png" onchange="validateFileType()"> <br>
