...
$result_Sub=mysqli_query($conn, "SELECT * FROM Sub_tb,Account_tb,Receive_tb WHERE 
Sub_tb.Sub_ID = Receive_tb.Sub_ID and
Account_tb.Acc_ID = Receive_tb.ACC_ID and
Receive_tb.Rec_No = '$Receive_No'")or die('Error In Session');
		...
		<div class="container">
			<div class="row">
				...
				<div class="no-print" align="right">
							<form action="Management-Receive-All.php">
				...
				<table width="100%" border="2" >
						<tr>
							...
							<td width="50%" align="center"><h3>ใบรับสินค้า</h3></td>
						</tr>
					...
					<table width="100%" >
						<tr>
							...
							<td width="33%" align="right"><h5><b>เลขที่เอกสาร :</b> <?php echo $Receive_No ?></h5></td>
						</tr>
						<tr>
							<td width="67%" align="left" colspan="2"><h5><b>ที่อยู่ : </b><?php echo $Sub_Address ?></h5></td>
							<td width="33%" align="right"><h5><b>วันที่ : </b> <?php echo $Rec_Date ?></h5></td>
						</tr>
						<tr>
							...
							<?php 
								...
								serial_tb.Ser_Withdraw = 'Insert-$Receive_No' group by device_tb.Project_ID";
					...
					<br>
					<h6 align="center">บริษัท เอสวีโอเอ จำกัด (มหาชน) ได้รับสินค้า เครื่องคอมพิวเตอร์และอุปกรณ์ต่างๆ ดังรายการต่อไปนี้</h6>
				</div>
				...
				<div class="col-sm-12">
					<table width="100%" id="customers" border="2">
						...
						$PerPage = 25;
							...
							$sql_Count="SELECT * FROM Receive_tb where Rec_No = '$Receive_No'";
							...
							$sql_GroupShow="Select * from serial_tb,Receive_tb where
							serial_tb.Ser_ID = Receive_tb.Ser_ID and
							Receive_tb.Rec_No = '$Receive_No'";
							...
							$Rec_Unit = $data_GroupShow['Rec_Unit'];
							$Rec_Remark = $data_GroupShow['Rec_Remark'];
							...
							$Dev_Model = $query_ModelShow['Dev_Model'];
									...
									<tr>
												...
												<td align="center"><?php echo $Rec_Unit;	?></td>
							...
							<table width="100%">
								<tr>
									...
									<td width="90%"><u><?php echo $Rec_Remark ?></u></td>
								</tr>
							</table><br><br>
							<p align="center">ได้ตรวจรับสินค้าตามรายการข้างบนนี้ครบถ้วนถูกต้อง</p>
							<table width="100%">
								<tr>
									<td width="50%" align="center"><b>ผู้ส่งสินค้า</b><br>ลงชื่อเจ้าหน้าที่หน่วยงาน</td>
...
//****************************************************************************************************************************/							
							...
							$sql_GroupShow="Select * from serial_tb,Receive_tb where
							serial_tb.Ser_ID = Receive_tb.Ser_ID and
							Receive_tb.Rec_No = '$Receive_No' group by serial_tb.Dev_ID";
							$query_GroupShow=mysqli_query($conn,$sql_GroupShow);
							$Num_Group = 0;
							while ($data_GroupShow=mysqli_fetch_array($query_GroupShow))     
							{ 
							...
							$sql_GetCount="SELECT * FROM serial_tb,Receive_tb where
							 serial_tb.Ser_ID = Receive_tb.Ser_ID and
						...
						<div class="header">
							<div class="float-left"><b>วันที่: </b><?php echo $Rec_Date ?></div>
							<div class="float-right"><b>เลขที่เอกสาร: </b><?php echo $Receive_No ?></div>
						</div>
						...
						<table width="100%" border="1">
							...
							$sql_GetModel="Select * from device_tb,Receive_tb where device_tb.Dev_ID =  Receive_tb.Dev_ID and Receive_tb.Rec_No = '$Receive_No' group by Receive_tb.Dev_ID";
							...
							while ($data_GetModel=mysqli_fetch_array($query_GetModel))     
							{
									...
									$sql_CountModel="Select * from Receive_tb where Receive_tb.Rec_No = '$Receive_No' and Dev_ID = '$DevID'";
									$query_CountModel=mysqli_query($conn,$sql_CountModel);
									$CountX=0;
									while ($data_CountModel=mysqli_fetch_array($query_CountModel)){$CountX = $data_CountModel['Rec_Unit']+$CountX;}
													...
													?>
																...
																<tbody>
																	<tr>
																		...
																		<td align="right"><?php echo $Rec_Unit ?></td>
																	</tr>
