  ...
  <div class="w3-row">
    ...
    <div class="w3-col m7">
      ...
      <div class="w3-container w3-card w3-white w3-round w3-margin"><br>
        <h2 class="w3-opacity" align="center">ข้อมูลการฝากอุปกรณ์ทั้งหมด</h2>
        <h4></h4><br>
        <hr class="w3-clear">
              <table width="100%" >
                    <thead>
                      <tr align="center">
                        <th width="30%">เลขที่เอกสารฝากอุปกรณ์</th>
                        <th width="30%">ชื่อผู้ฝาก-เบอร์ติดต่อ</th>
                        <th width="30%">ชื่อผู้รับฝาก</th>
                        <th width="15%" colspan="3">Tool</th>
                      </tr>
                      </thead>
                    </div>
                      <?php 
                      $sql_Si="Select * from account_tb,deposit_tb,deposit_detail_tb,deposit_receive_tb where 
                      account_tb.Acc_ID = deposit_receive_tb.Acc_ID and
                      deposit_tb.Dep_ID = deposit_detail_tb.Dep_ID and
                      deposit_detail_tb.DEPD_ID = deposit_receive_tb.DEPD_ID
                      group by deposit_receive_tb.DR_No ORDER BY DR_Date DESC";
                      $query_Si=mysqli_query($conn,$sql_Si);
                      while ($data_Si=mysqli_fetch_array($query_Si))                       
                      { 
                      $Dep_ID = $data_Si["Dep_ID"];
                      $Dep_Name = $data_Si["Dep_Name"];
                      $Dep_Tel = $data_Si["Dep_Tel"];
                    ...
                    <tbody>
                      <tr >                      
                        <td align="left" ><?php echo  $DR_No; ?></td>
                        <td align="center" ><?php echo $Dep_Name.'-'.$Dep_Tel;?></td>
                        <td align="center" ><?php echo $Acc_Name;?></td>
                        <form action="Management_Report_Deposit.php" method="POST">
                        <input type="text" hidden  id="DR_No" name="DR_No" value="<?php echo $DR_No ?>"></p>
                        <td align="center" >
