...
if(isset($_POST['Dep_IDX']))
{
  $_SESSION['Dep_IDX'] = $_POST['Dep_IDX'];
  
}
$Dep_ID = $_SESSION['Dep_IDX'];
$result_Pro=mysqli_query($conn, "SELECT * FROM Deposit_tb WHERE Dep_ID='$Dep_ID'")or die('Error In Session');
...
if(isset($_POST['Save']))
{
  $Dep_IDZ = $_POST['Dep_IDZ'];
  $Brand_ID = $_POST['Brand_ID'];
  $Type_ID = $_POST['Type_ID'];
  $DEPD_Model = $_POST['DEPD_Model'];
  $DEPD_Type = $_POST['DEPD_Type'];

      $Check_ID = "Select * From Deposit_Detail_tb ORDER BY DEPD_ID DESC LIMIT 1";
      $result_Check_ID =mysqli_query($conn,$Check_ID);
      $row_Check_ID =mysqli_fetch_array($result_Check_ID);
      $Last_id = $row_Check_ID['DEPD_ID'];
      ...
      $DEPD_ID = $ref.''.$ars;
        ...
        $SQL_AddDev ="INSERT INTO `Deposit_Detail_tb` (`DEPD_ID`,`Brand_ID`,`Type_ID`,`DEPD_Model`, `DEPD_Type`, `Dep_ID`) VALUES 
        ('$DEPD_ID', '$Brand_ID','$Type_ID','$DEPD_Model','$DEPD_Type','$Dep_IDZ')";
        if ($conn->query($SQL_AddDev)==TRUE)
          {
            echo '<script type="text/javascript">';
            echo 'alert("บันทึกข้อมูลอุปกรณ์ลงข้อมูลการฝากเรียบร้อย");';
  ...
  <div class="w3-row">
    ...
    <div class="w3-col m7">
      ...
      <div class="w3-container w3-card w3-white w3-round w3-margin"><br>
          <h4 align="center">เพิ่มอุปกรณ์สำหรับฝาก</h4>
          <form action="Management-Add-Deposit-Device.php" method="POST">
               
                <p>ชื่อผู้ฝาก
                <input type="text" class="form-control" Disabled id="Dep_Name" name="Dep_Name" value="<?php echo $Dep_Name ?>"></p>
                <p>เบอร์ติดต่อ
                <input type="text" class="form-control" Disabled id="Dep_Tel" name="Dep_Tel" value="<?php echo $Dep_Tel ?>">
                ...
                <p>ยี่ห้ออุปกรณ์
                  ...
                  <p>รุ่นอุปกรณ์
                <input type="text" class="form-control" placeholder="รุ่นอุปกรณ์" id="DEPD_Model" name="DEPD_Model" value=""></p>

                <p>ข้อมูล Serial-Number<br>
                    <input type="radio" name="DEPD_Type" value="None">  ไม่ระบุ Serial-Number<br>
                    <input type="radio" name="DEPD_Type"  value="Fix">  ระบุ Serial-Number
                  </p>

                  <p>จำนวนอุปกรณ์
                <input type="number" class="form-control" min="1" id="Project_Name" name="Project_Name" value="<?php echo $Project_Name ?>"></p>


                  <hr>

                  <input type="text" class="form-control" hidden  name="Dep_IDZ" value="<?php echo $Dep_ID ?>">
