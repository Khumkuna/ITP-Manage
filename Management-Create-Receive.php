  ...
  <div class="w3-row">
    ...
    <div class="w3-col m7">
      ...
      <div class="w3-container w3-card w3-white w3-round w3-margin"><br>
        <h2 class="w3-opacity" align="center">สร้างรายการรับคืนอุปกรณ์</h2>
        <hr class="w3-clear">

              <form action="Management-Select-Receive.php" method="POST">
                <div class="w3-container w3-padding">
                <p>เลขที่เอกสารรับคืน
                <input type="text" class="form-control" placeholder="เลขที่เอกสารรับคืน" id="Receive_No" required="required" name="Receive_No" value=""></p>

                <p>เลือกผู้ส่งคืน
                    <select name="Sub_ID" class="form-control" required="required">
                    <option value="">Select Sub-Contract</option>
                    <?php 
                    $query ="SELECT Sub_ID, Sub_Name, Sub_Type FROM sub_tb";
                    $result = $conn->query($query);
                    if($result->num_rows> 0){
                        while($OptionType=$result->fetch_assoc()){
                        $Sub_Name =$OptionType['Sub_Name'];
                        $Sub_ID =$OptionType['Sub_ID'];
                        $Sub_ID =$OptionType['Sub_ID'];
                        $Sub_Type =$OptionType['Sub_Type'];
                    ?>
                    <option value="<?php echo $Sub_ID; ?>" ><?php echo $Sub_Name.' ( '.$Sub_Type.' ) ' ?> </option>
                    <?php
                    }}
                    ?>
                  </select></p>
                  ...
                  <p>หมายเหตุการคืน
                  <input type="text" class="form-control" placeholder="หมายเหตุการคืน" id="Receive_Remark" name="Receive_Remark" value=""></p>


    
                <p>วันที่ทำการคืน
                <input type="date" class="form-control"  id="Receive_Date" name="Receive_Date" value="" required="required"></p>
