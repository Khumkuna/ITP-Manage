...
{
  $Receive_No = $_POST['Receive_No'];
  $Sub_ID = $_POST['Sub_ID'];

  $Receive_Date = $_POST['Receive_Date'];
  $CuteDate = explode('-',$Receive_Date);
  $CYear = $CuteDate[0]+543;
  $CMonth = $CuteDate[1];
  $CDay = $CuteDate[2];

  $Receive_Date = $CDay.'/'.$CMonth.'/'.$CYear;
  ...
  foreach($_POST['Ser_IDX'] as $Ser_IDX => $ValueS)
  {
    ...
    $Check_ID = "Select * From receive_tb ORDER BY Rec_ID DESC LIMIT 1";
      ...
      $SQL_AddReceive ="INSERT INTO `receive_tb` (`Rec_ID`,`Rec_No`,`Rec_Date`,`Rec_Unit`, `Rec_Remark`, `Ser_ID`, `ACC_ID`, `Sub_ID`, `Dev_ID`) 
      VALUES ('$Rec_ID', '$Receive_No','$Receive_Date','$SurUnit','$Receive_Remark','$ValueS','$Acc_ID ','$Sub_ID','$DevID')";
      if ($conn->query($SQL_AddReceive)==TRUE)
        {


              $Receive_NoX = 'Insert-'.$Receive_No;
              $SQL_Serial = "UPDATE serial_tb SET Ser_Out='$Show_Total',Ser_Withdraw='$Receive_NoX' WHERE Ser_ID='$ValueS'";
...
<div class="w3-container w3-content" style="max-width:1400px;margin-top:80px">    
  ...
  <div class="w3-row">
    ...
    <div class="w3-col m7">
      ...
      <p>
      <h4 class="w3-opacity" align="center">ข้อมูลการรับคืนอุปกรณ์ </h4></p>
                    ...
                    <tbody>
                            <tr>
                              ...
                              { ?>
                               ...
                               <td align="center"><input type="Number" min="1" max="<?php echo $Ser_Out?>" name="Ser_UnitX[]" value="" >
